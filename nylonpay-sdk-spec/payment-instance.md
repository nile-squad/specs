...
## Events
- `processing`: the payment is accepted and in flight (covers `pending`, `processing`, and `on_hold` statuses; to the merchant they are the same lifecycle moment of in-flight transactions). Fires at most once per instance. It fires on the next tick after creation when the initiation response is non-terminal, so handlers registered after the operation returns still fire. It also fires on the first poll that reports an in-flight status. A payment that resolves between polls (pending → successful) MUST still fire `processing` before the terminal event. For payouts specifically, `on_hold` indicates review-stage processing and polling continues automatically; use `transaction?.statusText` for human-readable status details when available.
