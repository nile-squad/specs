...
| `listTransactions` | `sdk` | `sdk-list-transactions` |
| `getTransactionsByTag` | `sdk` | `sdk-list-transactions` (same action as `listTransactions`; the SDK composes `tags: [tag]` client-side, see [Operations](./operations.md#gettransactionsbytag)) |
...
payload for signing (JCS, see [Security](./security.md#request-signing)) operates on the serialized body,
...
[Request Signing](./security.md#request-signing); `_fingerprint` is injected into the payload by the
...
conformance vectors V1–V7, server-side checks), response verification, and
the response size bound are consolidated in
[Security](./security.md). This page covers the wire contract: endpoint,
envelope, action payloads, response format, retry, and polling.

Every request is signed with HMAC-SHA256 over the inner `payload` object
(the operation input plus `_fingerprint`), NOT the full envelope. The four
`x-nylon-*` headers are computed per [Request Signing](./security.md#request-signing).
...
- **Jittered interval.** Each interval is the configured poll interval plus a small random jitter. The jitter keeps a fleet of concurrent instances from synchronising into a thundering herd against the status endpoint.
